...
class ACLWebCrawler extends AbstractCrawler {
	 ...
	 * @param conferences The abbreviations (see {https://aclanthology.info/}) of the conferences to scrape papers/authors from. null to scrape all. Does not work when only scraping authors
	 */
	public ACLWebCrawler(int beginYear, int endYear, String... conferences) {
		startURLAuthors = String.format("https://aclanthology.coli.uni-saarland.de/catalog/facet/author?"// get a list of all authors
				...
				beginYear, endYear);
		startURLPaper = "paper";//String.format("https://aclanthology.coli.uni-saarland.de/catalog?per_page=100&range[publish_date][begin]=%s&range[publish_date][end]=%s&search_field=title", beginYear, endYear);
	...
	private Set<String> selector(Set<String> all,String[] conferences,int from,int to){
		...
		if(conferences.length != 0) {
			selection = all.stream()
					.filter(p -> stringContainsItemFromList(p,conferences)&&containsYear(p,from2,to2)).collect(Collectors.toSet());

		}else {
			selection = all.stream()
					.filter(p -> containsYear(p,from2,to2)).collect(Collectors.toSet());
	...
	private ArrayList<Document> fetchWebpages(String startURL) throws IOException {
		...
		String[] conf = {"acl"};
		//this.conferences,from,to
		converenceURLs=selector(converenceURLs, conf, from, to);
		...
		if(startURL.equals("paper")) {
			selectedURLs = allURLs.stream().filter(p -> p.contains("https://aclweb.org/anthology/papers/")&& !p.endsWith(".bib")).collect(Collectors.toSet());

		}else {

			selectedURLs = allURLs.stream().filter(p ->  p.contains("https://aclweb.org/anthology/people/")).collect(Collectors.toSet());
	...
	private ArrayList<Person> extractAuthors(ArrayList<Document> webpages) {
		...
		for (Document doc : webpages) {
			...
			Person author = runWithDuplicateAvoidance ? Person.findOrCreateDbOrList(null, authorNameElement.text(), createdPersons) : new Person();
	...
	private ArrayList<Paper> extractPapers(ArrayList<Document> webpages) {
		...
		for (Document doc : webpages) {
			...
			if (!doc.title().contains("VOLUME")) {// VOLUMES/Overview-Pdfs are also part of the search-result and removed here
				//check is not earlier because the elmnt is needed
				if(conferences.length != 0 && !shouldSavePaper(doc)) //TODO: verstehen was sie hier mit den Konferenzen wollen
					continue; //innerLoop; //label is not needed necessarily, but helps readability

				Paper paper = runWithDuplicateAvoidance ? Paper.findOrCreateDbOrList(null, doc.title(), createdPapers) : new Paper();;
	...
	private ArrayList<Paper> extractPaperAuthor(List<Document> webpages) {
		...
		for (Document doc : webpages) {
			...
			if (!doc.title().contains("VOLUME")) {
				...
				String anthology = paperInformationElements.get(0).text();//splitRawTitle[0].replace("[", "").replace("]", "");
				...
				String remoLi = "http://aclweb.org/anthology/" + anthology;
				//System.out.println(remoLi);
				paper.setRemoteLink(remoLi); //wow that was easy
				paper.setReleaseDate(extractPaperRelease(doc));
					try {
						ExtractedMetadata meDa = scienceParse(parser, new URL(remoLi));
						...
						System.out.println("Fehlerhafter Link: " + remoLi);
						e.printStackTrace();
					}
	...
	/**
	 * Checks with the given conferences whether or not to save this paper into the database
	 * paper The web element of the paper to check
	 ...
	 * paper The web element of the paper to get the release year+month of
